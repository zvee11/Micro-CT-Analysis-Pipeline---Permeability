...
def main():
    ...
    ap.add_argument("--macro",
                    default=r"C:\Users\99619\Desktop\SVETA\Micro-CT-Analysis-Pipeline\geodict_lir_job.py",
    ...
    # Resolve run_id: use the one given, else the most recent run in the DB.
    run_id = args.run_id
    ...
    try:
        ...
        for (track, scan, conn), domains in sorted(groups.items(), key=_sort_key):
            ...
            # --skip-scans excludes the listed ones. No default skip (so the dry
            # scan 0, which carries the absolute permeability, runs unless asked
            # otherwise).
            ...
            for dtype in present:
                ...
                # result_summary.json with a non-null permeability), unless --force.
                scan_dir = dom["path"].parent.parent
                existing_summary = (scan_dir / "geodict" / dom["path"].stem
                                    / "result_summary.json")
                if not args.force and existing_summary.exists():
