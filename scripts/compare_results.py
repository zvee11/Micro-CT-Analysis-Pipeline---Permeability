...
def fetch_kr_table(con, run_id, connectivity):
    ...
    for track, scan, conn, sim_type, k_z, sw in rows:
        if sim_type == "absolute":
            # the absolute (dry-rock) permeability comes from the 'absolute'
            # domain; it is the kr denominator, kept per track.
            k_abs[(track, conn)] = k_z
        else:
            # gas/water at ANY scan (including scan 0, a real low-Sw timestep)
            # is a relative-perm measurement.
...
def load_parente_xlsx(path: Path):
    ...
    # Modified-Corey parameters (row 5, cols 55-60: Swc, Snr, krw0, krn0, nw, nn)
    corey = None
    swc = _num(ws.cell(5, 55).value)
    snr = _num(ws.cell(5, 56).value)
    krw0 = _num(ws.cell(5, 57).value)
    krn0 = _num(ws.cell(5, 58).value)
    nw = _num(ws.cell(5, 59).value)
    nn = _num(ws.cell(5, 60).value)
    if None not in (swc, snr, krw0, krn0, nw, nn):
        corey = {"Swc": swc, "Snr": snr, "krw0": krw0, "krn0": krn0,
                 "nw": nw, "nn": nn}
    return pts, kabs_mD, corey


def corey_curves(corey, n=100):
    """Return (Sw, krw, krn) arrays from the modified-Corey model.
        Se   = (Sw - Swc) / (1 - Swc - Snr)
        krw  = krw0 * Se**nw                 (wetting = brine)
        krn  = krn0 * (1 - Se)**nn           (non-wetting = gas)
    """
    import numpy as np
    swc, snr = corey["Swc"], corey["Snr"]
    sw = np.linspace(swc, 1.0 - snr, n)
    se = (sw - swc) / (1.0 - swc - snr)
    se = np.clip(se, 0.0, 1.0)
    krw = corey["krw0"] * se ** corey["nw"]
    krn = corey["krn0"] * (1.0 - se) ** corey["nn"]
    return sw, krw, krn
...
def main():
    ...
    parente = parente_kabs_mD = corey = None
    if args.parente:
        ppath = Path(args.parente)
        if not ppath.exists():
            print(f"WARNING: Parente workbook not found at {ppath}\n"
                  f"         (pass the correct path with --parente). "
                  f"Proceeding with pipeline data only.")
        else:
            parente, parente_kabs_mD, corey = load_parente_xlsx(ppath)
    ...
    _write_figures(kr_rows, kabs_rows, parente, corey)
...
def _write_figures(kr_rows, kabs_rows, parente, corey=None):
    try:
        import matplotlib
        matplotlib.use("Agg")
    ...
    # ---- kr vs Sw (SCATTER; kr is a cloud of independent points, not a curve) ----
    fig, ax = plt.subplots(figsize=(7, 5))
    mine = [(r["Sw"], r["kr_gas"], r["kr_brine"]) for r in kr_rows if r["Sw"] is not None]
    if mine:
        ax.scatter([m[0] for m in mine if m[1] is not None],
                   [m[1] for m in mine if m[1] is not None],
                   marker="o", color="tab:orange", label="kr_gas (pipeline)", zorder=3)
        ax.scatter([m[0] for m in mine if m[2] is not None],
                   [m[2] for m in mine if m[2] is not None],
                   marker="s", color="tab:blue", label="kr_brine (pipeline)", zorder=3)
    if parente:
        ...
        if pg:
            ax.scatter([x for x, _ in pg], [y for _, y in pg], marker="o",
                       facecolors="none", edgecolors="orange", alpha=0.7,
                       label="kr_gas (Parente)", zorder=2)
        if pw:
            ax.scatter([x for x, _ in pw], [y for _, y in pw], marker="s",
                       facecolors="none", edgecolors="navy", alpha=0.7,
                       label="kr_brine (Parente)", zorder=2)
    if corey:
        sw_c, krw_c, krn_c = corey_curves(corey)
        ax.plot(sw_c, krn_c, "-", color="darkorange", alpha=0.8,
                label="Corey kr_gas (Parente fit)", zorder=1)
        ax.plot(sw_c, krw_c, "-", color="darkblue", alpha=0.8,
                label="Corey kr_brine (Parente fit)", zorder=1)
