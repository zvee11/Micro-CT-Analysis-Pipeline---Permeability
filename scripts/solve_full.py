...
WSL_MPLBM_DIR   = "/root/MPLBM-UT"       # >>> CHANGED: root-only WSL (no /home user)
WSL_VENV        = "/root/mplbm_env"      # >>> CHANGED: venv under /root
WSL_WORK_DIR    = "/root/mplbm_workdir"  # >>> CHANGED: workdir under /root
CONNECTIVITY    = "18N"     # >>> CHANGED: was 26N; your analysed run is 18N (flip if running 26N)
MPLBM_MAX_ITER  = 200_000
MPLBM_CONVERGE  = 0.01      # >>> CHANGED: was 0.001; matches GeoDict ErrorBound=0.01
...
# >>> CHANGED: parse the voxel size from a domain filename, e.g.
#   domain_gas_02-8_4.99676um_8bu_750x750x273.raw  -> 4.99676e-6 m
# This matches how the GeoDict batch determined voxel size, so the MPLBM
# permeability is on the SAME footing for the cross-validation. Falls back to
# the DB/acquisition value only if the filename has no _<voxel>um_ token.
_VOXEL_RE = re.compile(r"_([0-9]*\.?[0-9]+)um_", re.IGNORECASE)

def voxel_m_from_name(path, fallback: float) -> float:
    m = _VOXEL_RE.search(str(path))
    if m:
        return float(m.group(1)) * 1e-6
    print(f"  WARNING: no _<voxel>um_ token in {getattr(path,'name',path)}; "
          f"using fallback {fallback:.4e} m")
    return fallback
...
def load_jobs(con: duckdb.DuckDBPyConnection,
    ...
    Rules:
    - First scan is INCLUDED (this run has no dry scan)   # >>> CHANGED
    ...
    rows = con.execute(f"""
        ...
        WHERE fb.connectivity = '{CONNECTIVITY}'
          {track_clause}   -- >>> CHANGED: removed 'scan_index > 0' so first scan is included
    ...
    for row in all_rows:
        ...
        for st in sim_types:
            if st == "absolute":
                ...
                # Gas: all qualifying timesteps except timestep X   # >>> CHANGED
                if is_X:
                    continue
...
cfg = {{
    ...
    'domain': {{
        ...
        'inlet and outlet layers': 10,   # >>> CHANGED: was 2; matches GeoDict pad=10
        'add mesh': False,
        'swap xz': True,
...
def run_job(job: dict, n_procs: int, wsl_work: str,
    ...
    voxel_m_job = voxel_m_from_name(job["win_path"], voxel_m)   # >>> CHANGED: per-domain voxel
    k_darcy = lbm_to_darcy(k_lbm, voxel_m_job)
    ...
    """, (
        ...
        sw,
        str(job["win_path"]), "",
        f"K_LBM={k_lbm:.6f} voxel_m={voxel_m_job:.4e} "   # >>> CHANGED: per-domain voxel
...
def main() -> None:
    ...
    # >>> CHANGED: handle root-only WSL. If ~ is used, expand against the real
    # home (/root for root, /home/<user> otherwise). Absolute paths pass through.
    _home = "/root" if wsl_user == "root" else f"/home/{wsl_user}"
    wsl_mplbm  = WSL_MPLBM_DIR.replace("~", _home)
    wsl_venv   = WSL_VENV.replace("~", _home)
    wsl_work   = WSL_WORK_DIR.replace("~", _home)
    ...
    mode = ask("\n  Mode", ["full","sample"], default="full")   # >>> CHANGED: default full
