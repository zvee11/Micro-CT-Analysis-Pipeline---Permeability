...
# NumPy 2.0 compatibility for older dependencies like ahds
...
def parse_dtype_from_header(header: str) -> type[np.generic]:
    """
    Parse the Avizo/Amira Lattice dtype token from a header string.

    This preserves the original fallback behaviour from favorite.py:
    if no dtype token is found, the volume is treated as uint8/byte.
    """
...
def parse_codec_from_header(header: str) -> str:
    """
    Parse the storage codec from an Avizo/Amira header.

    Returns one of: "raw", "hxzip", or "byterle".
    """
...
def parse_spacing_from_header(
    header: str,
    shape_zyx: tuple[int, int, int],
) -> tuple[float, float, float] | None:
    """
    Parse voxel spacing from the Avizo BoundingBox field.

    Parameters
    ----------
    header:
        Avizo/Amira text header.
    shape_zyx:
        Volume shape in NumPy order: (Z, Y, X).

    Returns
    -------
    tuple or None
        Spacing in metres/physical header units in NumPy order: (sz, sy, sx).
        Returns None when the header has no BoundingBox field.
    """
...
def label_histogram(vol: np.ndarray) -> dict[int, int]:
    """
    Count labels in a 3D integer volume without creating a second full mask.

    The implementation scans slice-by-slice, matching the memory-safe pattern
    already used in the existing logging code.
    """
    if vol.ndim != 3:
